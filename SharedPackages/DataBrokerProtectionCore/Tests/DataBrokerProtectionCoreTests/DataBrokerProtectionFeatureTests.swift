...
final class DataBrokerProtectionFeatureTests: XCTestCase {
    ...
    func testWhenParseActionCompletedFailsOnParsing_thenDelegateSendsBackTheCorrectError() async {
        ...
        let sut = DataBrokerProtectionFeature(delegate: mockCSSDelegate, executionConfig: BrokerJobExecutionConfig(), shouldContinue: { true })
    ...
    private func verifyTimeoutBehavior(
        ...
        let sut = DataBrokerProtectionFeature(delegate: mockCSSDelegate, executionConfig: executionConfig, shouldContinue: { true })
