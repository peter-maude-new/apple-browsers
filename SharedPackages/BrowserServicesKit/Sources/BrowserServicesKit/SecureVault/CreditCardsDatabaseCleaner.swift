//
//  CreditCardsDatabaseCleaner.swift
//
...
public struct CreditCardsCleanupError: Error {
    public let cleanupError: Error

    public static let syncActive: CreditCardsCleanupError = .init(cleanupError: CreditCardsCleanupCancelledError())
}

public struct CreditCardsCleanupCancelledError: Error {}

public final class CreditCardsDatabaseCleaner {
    ...
    public convenience init(
        ...
        errorEvents: EventMapping<CreditCardsCleanupError>?
    ) {
        self.init(
    ...
    init(
        ...
        cleanupCancellable = Publishers
            ...
            .sink { [weak self] cancelIfActive in
                self?.removeSyncableCreditCardsMetadataPendingDeletion(skipWhenSyncIsActive: cancelIfActive)
            }
    }
    ...
    func removeSyncableCreditCardsMetadataPendingDeletion(skipWhenSyncIsActive: Bool = true) {
            ...
            while true {
                do {
                    ...
                    if numberOfDeletedEntries == 0 {
                        Logger.secureVault.debug("No syncable credit cards metadata pending deletion")

                    } else {
                        Logger.secureVault.debug("Successfully purged \(numberOfDeletedEntries, privacy: .public) sync credit cards metadata")
                    }
                    break
                } catch {
    ...
    private static func removeSyncMetadataPendingDeletion(in database: Database) throws -> Int {
        let deletedRecords = try SecureVaultModels.SyncableCreditCardsRecord
            .filter(SecureVaultModels.SyncableCreditCardsRecord.Columns.objectId == nil)
    ...
    private let errorEvents: EventMapping<CreditCardsCleanupError>?
    ...
    private let workQueue = DispatchQueue(label: "CreditCardsDatabaseCleaner queue", qos: .userInitiated)
