//
//  IdentitiesDatabaseCleaner.swift
//
...
public struct IdentitiesCleanupError: Error {
    public let cleanupError: Error

    public static let syncActive: IdentitiesCleanupError = .init(cleanupError: IdentitiesCleanupCancelledError())
}

public struct IdentitiesCleanupCancelledError: Error {}

public final class IdentitiesDatabaseCleaner {
    ...
    public convenience init(
        ...
        errorEvents: EventMapping<IdentitiesCleanupError>?
    ) {
        self.init(
    ...
    init(
        ...
        cleanupCancellable = Publishers
            ...
            .sink { [weak self] cancelIfActive in
                self?.removeSyncableIdentitiesMetadataPendingDeletion(skipWhenSyncIsActive: cancelIfActive)
            }
    }
    ...
    func removeSyncableIdentitiesMetadataPendingDeletion(skipWhenSyncIsActive: Bool = true) {
            ...
            while true {
                do {
                    ...
                    if numberOfDeletedEntries == 0 {
                        Logger.secureVault.debug("No syncable identities metadata pending deletion")

                    } else {
                        Logger.secureVault.debug("Successfully purged \(numberOfDeletedEntries, privacy: .public) sync identities metadata")
                    }
                    break
                } catch {
    ...
    private static func removeSyncMetadataPendingDeletion(in database: Database) throws -> Int {
        let deletedRecords = try SecureVaultModels.SyncableIdentitiesRecord
            .filter(SecureVaultModels.SyncableIdentitiesRecord.Columns.objectId == nil)
    ...
    private let errorEvents: EventMapping<IdentitiesCleanupError>?
    ...
    private let workQueue = DispatchQueue(label: "IdentitiesDatabaseCleaner queue", qos: .userInitiated)
