...
/// A helper for registering a test delegate and creating a mock message based on the
/// incoming dictionary (which represents a message coming from a webview)
///
/// - Parameter message: The incoming message
///
func setupWith(message: [String: Any]) -> (UserScriptMessageBroker, UserScriptMessageBroker.Action, UserScriptMessage) {
    ...
    // Create a simple mock message - no unsafe WebKit mocking needed!
    let mockMessage = MockUserScriptMessage(
        name: testee.context,
        body: message,
        host: "example.com"
    )
    ...
    let action = testee.messageHandlerFor(mockMessage)

    return (testee, action, mockMessage)
}

// MARK: - Mock Types

/// A simple mock implementation of UserScriptMessage for testing.
/// This is a plain Swift struct - no unsafe memory manipulation or ObjC runtime tricks needed.
...
struct TestDelegate: Subfeature {
    ...
    /// An example of how to provide different handlers based on method name
    ...
    func notifyExample(params: Any, original: UserScriptMessage) async throws -> Encodable? {
        print("not replying...")
        return nil
    }
    ...
    func errorExample(params: Any, original: UserScriptMessage) async throws -> Encodable? {
    ...
    func responseExample(params: Any, original: UserScriptMessage) async throws -> Encodable? {
