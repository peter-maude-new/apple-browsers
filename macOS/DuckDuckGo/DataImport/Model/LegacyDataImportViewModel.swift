//
//  LegacyDataImportViewModel.swift
//
...
struct LegacyDataImportViewModel {
    ...
    init(importSource: Source? = nil,
         screen: Screen? = nil,
         availableImportSources: [DataImport.Source] = DataImport.Source.allCases.filter { $0.canImportData && $0 != .fileImport },
    ...
    mutating func goBack() {
        ...
        screen = importSource.legacyInitialScreen
        summary.removeAll()
    }
...
extension DataImport.Source {
    var legacyInitialScreen: LegacyDataImportViewModel.Screen {
        switch self {
...
extension LegacyDataImportViewModel {
    ...
    var isImportSourcePickerDisabled: Bool {
        importSource.legacyInitialScreen != screen || importTask != nil
    }
        ...
        return AsyncStream {
            for await event in importTask.progress {
                ...
                case .progress(let update):
                    ...
                    // on completion returns new LegacyDataImportViewModel with merged import summary
    ...
    @MainActor var actionButton: ButtonType? {
        ...
        case .fileImport where screen == importSource.legacyInitialScreen:
    ...
    var secondaryButton: ButtonType? {
        if importTask == nil {
            switch screen {
            case importSource.legacyInitialScreen, .feedback:
                return .cancel
