...
final class VPNSubscriptionEventsHandler {
    ...
    private func checkEntitlements() {
        Task {
            if let hasEntitlement = try? await subscriptionManager.isFeatureEnabled(feature: .networkProtection) {
    ...
    private func subscribeToWakeNotifications() {
        NSWorkspace.shared.notificationCenter
            ...
            .sink { [weak self] _ in
                ...
                Task { [weak self] in
                    if let hasEntitlement = try? await self?.subscriptionManager.isFeatureEnabled(feature: .networkProtection) {
