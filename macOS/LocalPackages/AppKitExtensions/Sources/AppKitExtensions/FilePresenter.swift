...
public class FilePresenter {
    ...
    public final var urlPublisher: AnyPublisher<URL?, Never> {
    ...
    public init(url: URL, consumeUnbalancedStartAccessingResource: Bool = false, createIfNeededCallback: ((URL) throws -> URL)? = nil) throws {
    ...
    /// - Parameter consumeUnbalancedStartAccessingResource: assume the `url` is already accessible (e.g. after choosing the file using Open Panel).
    ///   would cause an unbalanced `stopAccessingSecurityScopedResource` call on the File Presenter deallocation.
    public init(url: URL, primaryItemURL: URL, createIfNeededCallback: ((URL) throws -> URL)? = nil) throws {
...
public final class BookmarkFilePresenter: FilePresenter {

    private var _fileBookmarkData: Data?
    public final var fileBookmarkData: Data? {
        lock.withLock {
            _fileBookmarkData
        }
    }
    ...
    public final var fileBookmarkDataPublisher: AnyPublisher<Data?, Never> {
    ...
    /// - Parameter consumeUnbalancedStartAccessingResource: assume the `url` is already accessible (e.g. after choosing the file using Open Panel).
    ///   would cause an unbalanced `stopAccessingSecurityScopedResource` call on the File Presenter deallocation.
    public override init(url: URL, consumeUnbalancedStartAccessingResource: Bool = false, createIfNeededCallback: ((URL) throws -> URL)? = nil) throws {
    ...
    /// - Parameter consumeUnbalancedStartAccessingResource: assume the `url` is already accessible (e.g. after choosing the file using Open Panel).
    ///   would cause an unbalanced `stopAccessingSecurityScopedResource` call on the File Presenter deallocation.
    public override init(url: URL, primaryItemURL: URL, createIfNeededCallback: ((URL) throws -> URL)? = nil) throws {
    ...
    public init(fileBookmarkData: Data) throws {
    ...
    public override func didSetURL(_ newValue: URL?, oldValue: URL?) {
...
public extension FilePresenter {
    ...
    func coordinateMove<T>(from url: URL? = nil, to: URL, with options2: NSFileCoordinator.WritingOptions = .forReplacing, using move: (URL, URL) throws -> T) throws -> T {
        guard let innerPresenter = innerPresenters.last, let url = url ?? self.url else { throw CocoaError(.fileNoSuchFile) }
...
public extension NSFileCoordinator {
    ...
    func coordinateMove<T>(from url: URL, to: URL, with options2: NSFileCoordinator.WritingOptions = .forReplacing, using move: (URL, URL) throws -> T) throws -> T {
        var result: Result<T, Error>!
        var error: NSError?
...
extension Logger {
    static var fileDownload = { Logger(subsystem: "File Download", category: "") }()
}
