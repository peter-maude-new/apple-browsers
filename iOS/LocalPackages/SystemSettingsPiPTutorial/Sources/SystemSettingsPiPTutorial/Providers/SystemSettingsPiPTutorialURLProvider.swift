//
//  SystemSettingsPiPTutorialURLProvider.swift
//  DuckDuckGo
//
...
final class SystemSettingsPiPTutorialURLProvider: SystemSettingsPiPTutorialURLManaging {
    ...
    public func register(_ provider: any PiPTutorialURLProvider, for destination: SystemSettingsPiPTutorialDestination) {
        providers[destination.identifier] = provider
    }

    func url(for destination: SystemSettingsPiPTutorialDestination) throws(SystemSettingsPiPTutorialURLProviderError) -> URL {
        guard let provider = providers[destination.identifier] else { throw .noProviderAvailable(destination: destination) }
        do {
            return try provider.pipTutorialURL()
        } catch {
