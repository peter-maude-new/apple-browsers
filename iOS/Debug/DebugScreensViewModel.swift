...
class DebugScreensViewModel: ObservableObject {
    ...
    var debugScreensDeps: DebugScreensDeps

    var pushController: ((UIViewController) -> Void)?

    var cancellables = Set<AnyCancellable>()
    private let screensProvider: DebugScreensProvider
    private var screens: [DebugScreen] { screensProvider() }

    init(dependencies: DebugScreen.Dependencies, debugScreensDeps: DebugScreensDeps, screensProvider: @escaping DebugScreensProvider) {
        self.dependencies = dependencies
        self.debugScreensDeps = debugScreensDeps
    ...
    func persistInspectibleWebViewsState() {
        let oldValue = debugScreensDeps.inspectableWebViewsManager.isEnabled
        debugScreensDeps.inspectableWebViewsManager.isEnabled = isInspectibleWebViewsEnabled

        if oldValue != isInspectibleWebViewsEnabled {
            debugScreensDeps.inspectableWebViewsManager.notifyToggled()
        }
    }

    func refreshToggles() {
        self.isInternalUser = dependencies.internalUserDecider.isInternalUser
        self.isInspectibleWebViewsEnabled = debugScreensDeps.inspectableWebViewsManager.isEnabled
    }
    ...
    func executeAction(_ screen: DebugScreen) {
        ...
        case .action(_, let action):
            ...
            debugScreensDeps.actionMessagePresenter.present(message: "\(screen.title) - DONE")
    ...
    func buildView(_ screen: DebugScreen) -> AnyView {
        ...
        case .controller, .action:
            assertionFailure("Unexpected view creation")
    ...
    func setCustomURL(_ url: URL?, for configuration: Configuration) {
        do {
            ...
            debugScreensDeps.actionMessagePresenter.present(message: "Failed to set custom URL: \(error.localizedDescription)")
        }
    }
    ...
    func fetchPrivacyConfiguration(completion: @escaping (Bool) -> Void) {
        debugScreensDeps.configurationFetcher.fetchPrivacyConfiguration(isDebug: true, forceRefresh: true) { [weak self] result in
            switch result {
    ...
    func fetchRemoteMessagingConfiguration() {
        debugScreensDeps.remoteMessagingRefresher.refresh()
    }
